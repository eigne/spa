...
class NextTokenGroup : public SuchThatTokenGroup, public std::enable_shared_from_this<NextTokenGroup> {
public:
    explicit NextTokenGroup(const std::vector<std::shared_ptr<Token> >& tokenList) : SuchThatTokenGroup(tokenList) {
    }
